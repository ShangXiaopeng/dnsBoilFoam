...
//    const volVectorField::Boundary& T =
//        mixture_.T().boundaryField();
...
//            const fvPatchVectorField& Uwall =
//                refCast<const fvPatchVectorField>(U[patchi]);
...
//	    const scalar mu(mixture_.rho1()*mixture_.nuModel1().nu());
	    ...
	    const scalar mu
	    (
		mixture_.rho1().value()*nu.value()
	    );
*/
//----------------------------------------------------------------------------//
/*
	    const fvPatchVectorField& Uwall = U[patchi];
	    vectorField Up(Uwall.patchInternalField());
...
//	    const fvPatchVectorField& Twall = T[patchi];
//	    vectorField Tp(Twall.patchInternalField());
	    ...
	    const scalar K1(2.*ks0*l*h/mu/Vm);
	    const scalar K2(sigma/2./n/kB);
		...
		(
		    min
		    (
			max(cos(theta0) - Foam::asinh(Ucl/K1)*Tp/K2, -1.0),
			1.0
		    )
		)
	    );
...
//---------------------------------------------------------------------------//
	    ...
	    scalarField Ca = mu*mag(Ucl)/sigma;

	    Foam::threePhaseInterfaceProperties::InverseHoffmanFunction InvHoffFuncTheta
	    (
		100.0
	    );

	    RiddersRoot RRInvHoffFuncTheta(InvHoffFuncTheta, 1.e-10);
	    scalar InvHoffFuncThetaroot = RRInvHoffFuncTheta.root(0, 65);

	    scalarField theta(nHatp.size(), 100.0);
	    forAll(Up, faceI)
	    {
	        theta[faceI] = HoffmanFunction(Ca[faceI] + InvHoffFuncThetaroot);
	    }
...
:
    ...
    (
        ...
        mixture.alpha1().mesh(),
        dimensionedVector("nHat", dimless, vector(0.0, 0.0, 0.0))
    ),
