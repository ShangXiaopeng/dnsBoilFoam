...
)
{
    ...
    for ( int i = 0; i < N; i++ )
    {
        ...
        // Check for no change (potential issue when using +SMALL)
        if ((C2.val - C1.val) == 0.0 && C1.val != intVal)
        {
            continue;
        }

        // Check if relative distance is oob
        if ( ((intVal - C1.val)*(C2.val - C1.val) < 0) || ((intVal - C2.val)*(C2.val - C1.val) > 0) )
        {
            continue;
        }
...
void MeshGraph::GetAdjacentCells
(
    std::vector<CellFacePair>& IntCellFacePairs,
    const fvMesh& Min,
...
)
{
    ...
    (
        std::vector<MeshGraph::CellFacePair>::iterator it =
            IntCellFacePairs.begin();
        it != IntCellFacePairs.end();
        it++
    )
    {

        const labelList& cFaces = Min.cells()[(*it).c1];
        forAll (cFaces, i)
        {
	    const label facei = cFaces[i];

            if(!Min.isInternalFace(facei))

	        continue;

	    else
	    {
	        label owner = Min.faceOwner()[facei];
	        label neighbour = Min.faceNeighbour()[facei];

	        if(owner != (*it).c1 && alpha1[owner] > intVal)
	        {
//	    	    adjacentField[owner] = 10.0;
		    adjCellsSet.insert(owner);
	        }

	        if(owner != (*it).c1 && alpha1[neighbour] > intVal)
	        {
//	    	    adjacentField[neighbour] = 10.0;
		    adjCellsSet.insert(neighbour);
	        }
	    }
        }
